...
public class UploadFileServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//获取项目存放文件目录
			File parent = DirectoryUtil.getProjectFileDataPath(request);
			
			/*构建文件目录以及目录文件*/
			File fileDir = new File(parent,"zixun"+"/"+fileFolder+ "/");
			if (!fileDir.exists()) {fileDir.mkdirs();}
			
			/*重命名文件*/
			String filename = UUID.randomUUID().toString();
			File savefile = new File(fileDir , filename + "." + extensionName);
			...
			newFileName = projectBasePath+"/".concat(DirectoryUtil.getProjectName(request)).concat("Files")+ "/" + "zixun" + "/" + fileFolder + "/" + filename + "." + extensionName;
