...
function initUploadFile(flag){
	...
	$("#maImg").uploadify({
		...
		'onUploadSuccess' : function(file, data, response) {
			$('input[name="processUse.puImg"]').val(data);
			$('#maImg').uploadify('disable', true);
			$('#prev').removeClass('disabled');
			$('#a_pre').css({'color':'white'});
...
function initProList(page,rows){
	...
	condition['page']=page;
	condition['rows']=rows;
	condition['processUse.recId'] = $('#h_recId').val();
	$('#proTab tr:not(:first)').remove();
	$.post('processUse_findPager.action?tt='+Math.random(),condition,function(result){
		...
		if(result){
			...
			for(var i=0;i<rowList.length;i++){
				var row = rowList[i];
				var puName = (row.puName==null||row.puName=="")?"":row.puName;
				var useTime = (row.useTime==null||row.useTime=="")?"":row.useTime;
				var useTotal = (row.useTotal==null||row.useTotal=="")?"":row.useTotal;
				var crttime = (row.crttime==null||row.crttime=="")?"":row.crttime;
...
//				var imgp = '<a rel="previewImg" href="'+fpath+maImg+'" title="'+niName+'"><img alt="预览" src="'+fpath+maImg+'" style="width:150px; height:60px;padding:2px;"></a>';
				...
				var content = '<tr '+(i%2==0?"":"class=\"odd\"")+'>\
			        ...
			        <a class="tablelink" onclick="deleteNodeinfo('+row.puId+')">删除</a>\
			        </td>\
			        </tr> ';
...
function deleteNodeinfo(id){
	if(id){
		var index = layer.confirm('确定删除信息?', {
		    ...
		    layer.close(index);
		    $.post('processUse_delete.action',{ids:id},function(result){
				layer.msg(result);
				initProList(1,10);
			},'TEXT');
		}, function(){
...
function submitForm(){
	...
	var params = {};
	var formUrl = 'processUse_add.action';
	if(urlType!=1){
		formUrl = 'processUse_update.action';
	}
	/*$.messager.progress({
		title : '提示',
		text : '数据处理中，请稍后....'
...
function updateRecord(index){
	...
	if(row){
		...
		$('.formtitle span').html('修改加工投入品信息');
		
		for(var item in row){
			$('input[name="processUse.'+item+'"]').val(row[item]);
		}
		
		try {
			if(row.puImg){
				initUploadFile(false);
				$("a#a_pre").attr('href','/nytsyFiles/element/'+row.puImg);
				$("a#a_pre").fancybox();
			}else{
				initUploadFile(true);
			}
		...
		$('#useTime').datebox('setValue',row.useTime);
