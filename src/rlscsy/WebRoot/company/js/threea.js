...
function initProList(page,rows){
	...
	condition['page']=page;
	condition['rows']=rows;
	condition['threea.recId'] = $('#h_recId').val();
	$('#proTab tr:not(:first)').remove();
	$.post('threea_findThreeaList.action?tt='+Math.random(),condition,function(result){
		
		if(result){
			gthreeaList = result.rows;
			//显示条数
			$('#total').html(result.total);
			$('#page').html(page);
			
			var threeaList = result.rows;
			//<td><input type="checkbox" class="proCheck" value="'+pro.proId+'" /></td>\
			for(var i=0;i<threeaList.length;i++){
				var threea = threeaList[i];
				var content = '<tr '+(i%2==0?"":"class=\"odd\"")+'>\
			        <td>'+threea.cuoshi+'</td>\
			        <td>'+threea.guicheng+'</td>\
			        <td>'+threea.jianyan+'</td>\
			        <td><a class="tablelink" onclick="updateRecord('+i+')">修改</a>\
			        <a class="tablelink" onclick="deleteRecord('+threea.thId+')">删除</a>\
			        </td>\
			        </tr> ';
...
function deleteRecord(id){
	if(id){
		var index = layer.confirm('确定删除信息?', {
		    ...
		    layer.close(index);
		    $.post('threea_deleteThreea.action',{ids:id},function(result){
				layer.msg(result);
				initProList(1,10);
			},'TEXT');
...
function submitForm(){
	...
	var params = {};
	var formUrl = 'threea_addThreea.action';
	if(urlType!=1){
		formUrl = 'threea_updateThreea.action';
	}
...
function updateRecord(index){
	var threea = gthreeaList[index];
	if(threea){
		addRecord(true);
		urlType = 2;
		$('.formtitle span').html('修改三品一标信息');
		$('input[name="threea.thId"]').val(threea.thId);
		$('input[name="threea.recId"]').val(threea.recId);
		
		$('textarea[name="threea.cuoshi"]').val(threea.cuoshi);
		$('textarea[name="threea.guicheng"]').val(threea.guicheng);
		$('textarea[name="threea.jianyan"]').val(threea.jianyan);
		$('textarea[name="threea.biaozhun"]').val(threea.biaozhun);
		$('textarea[name="threea.shengming"]').val(threea.shengming);
