...
$(function(){
	...
	$.ajax( {
		...
		data : {'useguide.ugNo':'meatOutInfoDetail_tz'},
		type : 'post',
		dataType : 'json',
...
function addRecord(flag){
	...
	$('.formtitle span').html('添加屠宰厂肉品交易明细信息');
	urlType = 1;
	$('#proDiv').hide();
...
function updateMarketIn(index){
	...
	if(meatOutInfoDetail){
		...
		$('.formtitle span').html('修改屠宰厂肉品交易明细信息');
...
function goHref(){
	...
	var ptbId=$('#ptbId').val();
	$("#goPro").attr("href","meatOutInfoBase_tz.jsp?ptbId="+ptbId+'&batchName='+encodeURI(encodeURI(batchName))); 
