...
$(function(){
	...
	$.ajax( {
		...
		data : {'useguide.ugNo':'meatOutInfoBase_tz'},
		type : 'post',
		dataType : 'json',
...
function addRecord(flag){
	...
	$('.formtitle span').html('添加屠宰交易基本信息');
	urlType = 1;
	$('#proDiv').hide();
...
function updateMarketIn(index){
	...
	if(meatOutInfoBase){
		...
		$('.formtitle span').html('修改屠宰交易基本信息');
...
function toDetail(moibId,tranDate){
	...
	window.location.href=$('#basePath').val()+"company/meatOutInfoDetail_tz.jsp?moibId="+moibId+'&ptbId='+ptbId+'&batchName='+encodeURI(encodeURI(batchName))+'&tranDate='+tranDate;
}