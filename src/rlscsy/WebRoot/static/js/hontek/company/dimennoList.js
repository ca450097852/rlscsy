...
$(function(){
	...
	$.post("ent_getLoginCompanyDimenno.action",{"entId":entId},function(result){
		if(result[0]){
			...
			if(obj.isbatch==0){
				$.post('proTypeQrcode_findProTypeQrcodebyEntId.action',{'proTypeQrcode.entId':$('#h_entId').val(),"entId":obj.entId},function(result){
					if(result){
					   ...
					   success: function(result){
				     		for(var j=0;j<result.length;j++){
								...
								$("#"+typeTabId).datagrid({  
								      ...
								      url:'proTypeQrcode_findProTypeBatchbyEntId.action',//url调用Action方法  
								      queryParams:{"proTypeQrcode.ptqId":ptqId,"entId":obj.entId},
								      loadMsg : '数据装载中......',  			      
								      fit:true,			      
