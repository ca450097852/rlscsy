	    	...
	    	if(result){
				...
				for(var i=0;i<recordEntList.length;i++){//第一次循环获取外层主体div
					...
					if(record){
						...
						var article_id = "ent_article_id"+i;
						fristHtml += record_1+article_id+record_2+record.recName+record_3;
					}
				}
				$(".wrap-container").append(fristHtml);//主体档案外层，html代码
	    				...
	    				if(batch){
	    					for(var k=0;k<batch.length;k++){
	    						...
	    						if(bat){
	    							...
	    							var article_id = "ptqb_article_id"+j+k;
	    							
	    							if(dimenno.length>14&&ptbcode==dimenno){
	    								secondHtml += record_1+article_id+record_2+batchName+record_3;
	    							}else if(dimenno.length<15){
	    								secondHtml += record_1+article_id+record_2+batchName+record_3;
	    							}
	    						}
	    					}
	    				...
	    				if(typeRecord){
							...
							if(dimenno.length>12){	//主体和具体分类
								if(objTypeId==1||recId==ptq_recId){
									var article_id = "ptq_article_id"+j;
									//header_1 + (article_id) + header_2 + (档案名) + header_3;
									secondHtml += record_1+article_id+record_2+typeRecord.recName+record_3;
								}
							}else{					//所有
								var article_id = "ptq_article_id"+j;
								secondHtml += record_1+article_id+record_2+typeRecord.recName+record_3;
				...
				for(var a=0;a<recordEntList.length;a++){//第二次循环获取主体档案内容
					//获取档案
					var record = recordEntList[a];
					if(record){
						var recId = record.recId;
						var objTypeId = record.objTypeId;
						var article_id = "ent_article_id"+a;
						findRecordElement(recId,article_id);//获取档案要素
					}
				}
				
	    		if(entisbatch!=0){//批次溯源
	    			for(var b=0;b<recordTypeList.length;b++){//第二次循环获取批次档案内容
	    				var typeRecord = recordTypeList[b];
	    				var batch = typeRecord.batch;//批次档案列表
	    				var elements = typeRecord.elements;//要素集合
	    				if(batch){
...
//	    							var recId = bat.recId;//档案id
//	    							var batchName = bat.batchName;//批次名称
//	    							var article_id = "article_id"+b+c;
//	    							decodePTBElement(recId,elements,article_id);
//	    						}
	    						...
	    						if(bat){
	    							...
	    							var recId = bat.recId;//档案id
	    							var batchName = bat.batchName;//批次名称
	    							var article_id = "ptqb_article_id"+b+c;
	    			...
	    			for(var b=0;b<recordTypeList.length;b++){//第二次循环获取分类档案内容
	    				var typeRecord = recordTypeList[b];
	    				if(typeRecord){
	    					var recId = typeRecord.recId;
							...
							if(dimenno.length>12){	//主体和具体分类
								if(recId==ptq_recId){//种类档案，则先获取分类基本信息
									...
									findPtqInfo(ptq,"ptq_article_id"+b);
								}
								findRecordElement(recId,"ptq_article_id"+b);//获取档案要素
							}else{					//所有
								var ptq = {};
								ptq["ptq.ptqId"]=objId;
								ptq["ptq.entId"]=thisEntId;
...
//解释批次档案要素
function decodePTBElement(recId,elements,divId){
...
function showIMG(src){
	...
	var image = '<img src="'+src+'" align="center" style="width: 100%"/>';
