...
headarr[1] = "<li><a href=\"#section-circlefill-1\" ><span>企业信息</span></a></li>";
sectionarr[1] = "<section id=\"section-circlefill-1\"></section>";
var nav_count = 2;//数组下标；
...
$(function() {
	...
	if(entCode!=''){
		//首先获取企业信息；
		var companyInfo_html = findCompanyInfo(entCode);
		//资质信息
		var zizhiInfo_html = findZizhiInfo(entCode);
		//监管信息
		var jianguan_html = findJianguanInfo(entCode);
		// 企业生产信息
		
		recordarr[1] = companyInfo_html+zizhiInfo_html+jianguan_html;
		...
		for(var i=1;i<nav_count;i++){
			if(headarr[i]){
				head_htmls += headarr[i];
				section_htmls += sectionarr[i];
			}
			
		}
		
//		if($('#validCode').val()==1){
//			head_htmls += '<li><a href="#section-circlefill-'+(nav_count+1)+'" ><span>二维码验证</span></a></li>';
//			section_htmls += '<section id="section-circlefill-'+(nav_count+1)+'" style="min-height:250px;text-align:left;">\
//							<input type="text" id="v_validCode" placeholder="请输入二维码标签上的四位验证码" maxlength="4" style="width:60%;padding:9px;">\
//							<a class="btn-sure" href="javascript:validCode()">确 定</a>\
//							<br><span id="validCodeMsg"></span>\
//							</section>';
//		}
		
		$("#head_html").html(head_htmls);
		$("#section_html").html(section_htmls);
		...
		if(companyRsts!='4'){//未审核
			showIMG("static/image/code/tips.png");
		}else if(iscompile==0){//未完善
			//showIMG("");
		}
	...
	if($('#validCode').val() == 1){
		var ind = layer.confirm('是否验证二维码？', {
		  btn: ['是','否'] //按钮
		}, function(){
			layer.close(ind);
			window.location.href="validCode.jsp?dimenno="+$('#dimenno').val();
		});
	}
...
function checkValidCode(){
	layer.prompt({title: '请输入标签上的四位验证码', formType: 3}, function(text, index){
	    $.ajax({
			url:'validCode_updateValidCode.action',
			type:'post',
			async:false,
			data:{'validCode.dimenno':$('#dimenno').val(),'validCode.validCode':text},
			success:function(result){
				layer.close(index);
				index = layer.alert(result,function(){
					layer.close(index);
					if(result.indexOf("验证通过")==-1){
						checkValidCode();
					}
				});
	    ...
	    success : function(data) {
			...
			for(var i=0;i<data.length;i++){
				...
				if(a){
					...
					zzUlHtml+= '<li><span><a href="javascript:void(0)" onclick="showIMG(\''+src+'\')" ><img src="'+src+'" width="80" height="80"/></a></span><p>'+appName+'</p></li>';
				}
			}
			
			if(zzUlHtml.length>0){
				
				res = html_4+html_1+"企业资质信息"+html_2+zzUlHtml+html_3;
...
function findJianguanInfo(entCode){
	...
	$.post('mbsupervise_findSuperviseList.action',cond,function(result){
		...
		if(jgUlHtml.length>0){
			res = html_4+html_1+"监管信息"+html_2+jgUlHtml+html_3;
				...
				if(appList.length>0){//存在图片
					...
					for(var j=0;j<appList.length;j++){
						...
						if(app){
		    				...
		    				UlHtml+= '<li><span><a href="javascript:void(0)" onclick="showIMG(\''+src+'\')" ><img src="'+src+'" width="80" height="80"/></a></span><p>'+appType+'</p></li>';
						}
...
function findCompanyRecord(){
	...
	$.ajax({
	    url:'portal_Elements_getRecordByEntId.action',
	    data:'entId='+thisEntId,
	    type:'post',
	    async : false,
	    dataType:'json',
	    success : function(result) {
	    	if(result){
	    		var recordEntList = result.ent;//主体档案列表
	    		var recordTypeList = result.type;//分类档案列表
	    		
	    		
				for(var i=0;i<recordEntList.length;i++){//第一次循环获取外层主体div
					//获取档案
					var record = recordEntList[i];
					if(record){
						var recId = record.recId;
						var objTypeId = record.objTypeId;//企业1；分类二维码2；产品3;4批次
						
						headarr[nav_count] = "<li><a href=\"#section-circlefill-"+nav_count+"\" ><span>"+record.recName+"</span></a></li>";
						sectionarr[nav_count] = "<section id=\"section-circlefill-"+nav_count+"\"></section>";
						recordarr[nav_count] = findRecordElement(recId);//获取档案要素
						nav_count = nav_count+1;
					}
				}
	    		
				if(entisbatch!=0){//批次溯源
	    		
					var secondHtml = "";//清除
	    			for(var j=0;j<recordTypeList.length;j++){//第一次循环获取外层批次档div
	    				var typeRecord = recordTypeList[j];
	    				var objId = typeRecord.objId;
	    				var batch = typeRecord.batch;//批次档案列表
	    				var elements = typeRecord.elements;//要素集合
	    				if(batch){
	    					/*获取当前分类信息*/
		    				var ptq = {};
							ptq["ptq.ptqId"]=objId;
							ptq["ptq.entId"]=thisEntId;
							var ptqhtml = findPtqInfo(ptq);
							/*获取当前分类信息*/
	    					...
	    					for(var k=0;k<batch.length;k++){
	    						...
	    						if(bat){
	    							...
	    							var typecode = ptbcode.substring(0, 14);
	    							var recId = bat.recId;//档案id
	    							var batchName = bat.batchName;//批次名称
	    							
	    							if(dimenno.length==16&&ptbcode==dimenno){//当前批次的档案
	    								headarr[nav_count] = "<li><a href=\"#section-circlefill-"+nav_count+"\" ><span>"+batchName+"</span></a></li>";
		    							sectionarr[nav_count] = "<section id=\"section-circlefill-"+nav_count+"\"></section>";
		    							recordarr[nav_count] = ptqhtml+decodePTBElement(recId,elements);//批次档案数据==分类基本信息+批次（分类）要素信息；
		    							nav_count = nav_count+1;
		    							
	    							}else if(dimenno.length<16&&typecode==dimenno){//当前分类下的所有批次的档案
	    								headarr[nav_count] = "<li><a href=\"#section-circlefill-"+nav_count+"\" ><span>"+batchName+"</span></a></li>";
		    							sectionarr[nav_count] = "<section id=\"section-circlefill-"+nav_count+"\"></section>";
		    							recordarr[nav_count] =  ptqhtml+decodePTBElement(recId,elements);//批次档案数据==分类基本信息+批次（分类）要素信息；
		    							nav_count = nav_count+1;
	    			...
	    			//$(".wrap-container").append(secondHtml);
    			
				}else{
	    			for(var j=0;j<recordTypeList.length;j++){//第一次循环获取外层种类div
	    				var typeRecord = recordTypeList[j];
	    				
	    				if(typeRecord){
	    					var recId = typeRecord.recId;
	    					var objId = typeRecord.objId;
	    					var objTypeId = typeRecord.objTypeId;//企业1；分类二维码2；产品3;4批次
							if(dimenno.length==14){	//主体和具体分类
								if(recId==ptq_recId){
									...
									var ptqhtml = findPtqInfo(ptq);
									var rcelhtml = findRecordElement(recId);//获取档案要素
									
									headarr[nav_count] = "<li><a href=\"#section-circlefill-"+nav_count+"\" ><span>"+typeRecord.recName+"</span></a></li>";
	    							sectionarr[nav_count] = "<section id=\"section-circlefill-"+nav_count+"\"></section>";
	    							recordarr[nav_count] = ptqhtml+rcelhtml;
	    							nav_count = nav_count+1;
	    							
									
									
								}
							}else if(dimenno.length==12){//所有
...
//解释批次档案要素
function decodePTBElement(recId,elements){
	var res_html = "";
		...
		for(var i=0;i<elements.length;i++){
			...
			if(a){
				...
				if(tableName=='tb_threea'){//三品一标
					var threea = findThreeaList(recId);
					if(threea==""){
						iscompile = 0;
					}else{
						res_html += threea;
					...
					var eartag = findEartagList(recId);
					if(eartag==""){
						iscompile = 0;
					}else{
						res_html += eartag;
					...
					var checkinfo = findCheckinfoList(recId);
					if(checkinfo==""){
						iscompile = 0;
					}else{
						res_html += checkinfo;
					...
					var agriinput = findAgriInputList(recId);
					if(agriinput==""){
						iscompile = 0;
					}else{
						res_html += agriinput;
					...
					var agriuse = findAgriUseList(recId);
					if(agriuse==""){
						iscompile = 0;
					}else{
						res_html += agriuse;
					...
					var gaininfo = findGaininfoList(recId);
					if(gaininfo==""){
						iscompile = 0;
					}else{
						res_html += gaininfo;
					...
					var saleinfo = findSaleinfoList(recId);
					if(saleinfo==""){
						iscompile = 0;
					}else{
						res_html += saleinfo;
					...
					var fertilizer = findFertilizerUseList(recId);
					if(fertilizer==""){
						iscompile = 0;
					}else{
						res_html += fertilizer;
					}
				}else if(tableName=='tb_node_info'){//生产节点信息
					var nodeinfo = findNodeinfoList(recId);
					if(nodeinfo==""){
						iscompile = 0;
					}else{
						res_html += nodeinfo;
					}
				}else if(tableName=='tb_drug_use'){//兽药使用信息
					var drugUse = findDrugUseList(recId);
					if(drugUse==""){
						iscompile = 0;
					}else{
						res_html += drugUse;
					}
				}else if(tableName=='tb_feed_use'){//饲料使用记录
					var feedUse = findFeedUseList(recId);
					if(feedUse==""){
						iscompile = 0;
					}else{
						res_html += feedUse;
			...
			for(var i=0;i<result.rows.length;i++){
				...
				if(massifList.length>0){
					for(var k=0;k<massifList.length;k++){
						...
						var imgStr = "无";
						if(maImg!=''){
							imgStr = "<a  href=\"javascript:void(0)\" onclick=\"showIMG('"+proimgDir+maImg+"')\" ><img src=\""+proimgDir+maImg+"\" width=\"50\" height=\"50\"/></a>";
						}
				...
				if(liList!=''){
					res += elements_1+"种类基本信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
				if(cddkList!=''){
					res += elements_1+"种类地块信息"+elements_2+cddkList+elements_3+line_tr;
					cddkList = "";
				}
			}
		}
...
//获取档案要素
function findRecordElement(recId){
	var res_html = "";
	$.ajax({
	...
	$.ajax({
 	    ...
 	    success : function(result) {
			...
			for(var i=0;i<result.rows.length;i++){
				...
				if(liList!=''){
					res += elements_1+"三品一标信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
		}
...
//				liList+="<li><span>佩戴时间：</span><p>"+wearTime+"</p></li>";
				...
				if(liList!=''){
					res += elements_1+"耳标信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
		}
...
//投入品购买信息
function findAgriInputList(recId){
	var res = "";
	...
	$.ajax({
 	    ...
 	    success : function(result) {
			...
			for(var i=0;i<result.rows.length;i++){
				...
				if(liList!=''){
					res += elements_1+"投入品购买信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
		}
...
//采摘收获信息
function findGaininfoList(recId){
	var res = "";
...
//				liList+="<li><span>录入时间：</span><p>"+a.crttime+"</p></li>";
				...
				if(liList!=''){
					res += elements_1+"采摘收获信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
		}
...
//				liList+="<li><span>录入时间：</span><p>"+a.crttime+"</p></li>";
				...
				if(liList!=''){
					res += elements_1+"销售信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
...
//获取兽药使用信息
function findDrugUseList(recId){
	var res = "";
	
	var drugUse = {};
	drugUse["drugUse.recId"]=recId;
	$.ajax({
   	    url:'portal_Elements_findDrugUseList.action',
   	    data:drugUse,
	    type:'post',
	    async : false,
	    dataType:'json',
	    success : function(result) {
			...
			for(var i=0;i<result.rows.length;i++){
				...
				havetag = a.havetag;
				havetag = havetag=="1"?"有":"无";
				liList+="<li><span>药品名称：</span><p>"+a.drugName+"</p></li>";
				liList+="<li><span>防治对象：</span><p>"+a.useObject+"</p></li>";
				liList+="<li><span>施用方式：</span><p>"+a.useWay+"</p></li>";
				liList+="<li><span>稀释浓度：</span><p>"+a.useDosage+"</p></li>";
				liList+="<li><span>施用量：</span><p>"+a.useTotal+"</p></li>";
				...
				if(liList!=''){
					res += elements_1+"兽药使用信息"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
		}
...
//获取饲料使用记录
function findFeedUseList(recId){
	var res = "";
	
	var feedUse = {};
	feedUse["feedUse.recId"]=recId;
	$.ajax({
	    url:'portal_Elements_findFeedUseList.action',
	    data:feedUse,
	    type:'post',
	    async : false,
	    dataType:'json',
	    success : function(result) {
			...
			for(var i=0;i<result.rows.length;i++){
				...
				liList+="<li><span>饲料名称：</span><p>"+a.feedname+"</p></li>";
				liList+="<li><span>使用目的：</span><p>"+a.purpose+"</p></li>";
				//liList+="<li><span>施用面积：</span><p>"+a.userarea+"</p></li>";
				liList+="<li><span>施用量：</span><p>"+a.usertotal+"</p></li>";
				liList+="<li><span>使用日期：</span><p>"+a.usedate+"</p></li>";
				liList+="<li><span>施用人：</span><p>"+a.useman+"</p></li>";
				if(liList!=''){
					res += elements_1+"饲料使用记录"+elements_2+liList+elements_3+line_tr;
					liList = "";
				}
			}
		}
